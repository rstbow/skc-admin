...
(async () => {
  ...
  const cols = await p.request().query(`
    ...
    FROM sys.columns c WHERE c.object_id = OBJECT_ID('admin.Brands') ORDER BY c.column_id;
  `);
  console.log('--- admin.Brands columns ---');
  for (const c of cols.recordset) {
    ...
    console.log('  ' + c.name.padEnd(24) + typ + (c.is_nullable ? '' : ' NOT NULL'));
  }
  const sample = await p.request().query(`SELECT TOP 5 BrandUID, BrandName, BrandID, IsActive FROM admin.Brands ORDER BY BrandName;`);
  console.log('\n--- current admin.Brands ---');
  for (const r of sample.recordset) console.log('  ', r);
  const cnt = await p.request().query(`SELECT COUNT(*) AS n FROM admin.Brands;`);
  console.log('\nTotal:', cnt.recordset[0].n);
  await p.close();
