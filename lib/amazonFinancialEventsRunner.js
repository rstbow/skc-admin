...
async function runAmazonFinancialEvents({
  ...
  try {
    ...
    const { pages, hitCap, capReason, elapsedMs } = await paginateSpApi(ctx, buildPath, (payload) => {
      ...
      // Log what each page returned so we can see if Amazon is returning
      // data at all vs the flatten/merge dropping it later
      const pageCounts = {};
      for (const k of Object.keys(events)) {
        if (Array.isArray(events[k])) pageCounts[k] = events[k].length;
      }
      console.log('[runner/fin-events] page returned', {
        nextToken: payload.NextToken ? 'yes' : 'no',
        topLevelKeys: Object.keys(payload),
        financialEventsKeys: Object.keys(events),
        counts: pageCounts,
      });
    }, { maxPages: PAGE_CAP, maxElapsedMs: TIME_CAP, pageDelayMs: 300 });

    /* Roll up raw counts before flatten */
    const rawCounts = {};
    for (const k of Object.keys(allEvents)) rawCounts[k] = allEvents[k].length;
    console.log('[runner/fin-events] post-paginate raw totals', { pages, rawCounts });

    /* Flatten event lists into TVP rows */
    const rawRows = flattenAll(allEvents);
    // Dedupe by natural key (EventType, ExternalID). When SP-API paging
    // overlaps or a retry re-fetches a page we can see the same event
    // twice. Proc-side we also dedupe via ROW_NUMBER (017c), but doing it
    // here saves a round-trip and keeps logs readable. "Last write wins"
    // is fine — duplicates should be bit-identical.
    const seen = new Map();
    for (const r of rawRows) seen.set(r.EventType + '|' + r.ExternalID, r);
    const rows = Array.from(seen.values());
    const deduped = rawRows.length - rows.length;
    console.log('[runner/fin-events] post-flatten row count', rows.length,
      deduped ? `(deduped ${deduped} duplicate natural keys)` : '');
    ...
    return {
      ...
      // Diagnostic — what Amazon actually returned per event list type
