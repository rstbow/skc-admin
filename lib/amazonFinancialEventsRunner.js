...
async function execMergeBatch(brandPool, brandUID, runID, rows) {
  ...
  // Decimals travel as NVARCHAR(20); the MERGE proc CASTs server-side.
  // Bypasses the tedious driver's decimal encoder entirely (which rejects
  // values with binary-float noise as "invalid scale").
