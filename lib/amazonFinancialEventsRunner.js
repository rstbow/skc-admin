...
async function pullMergeWindow(ctx, brandPool, runID, win, opts) {
  ...
  const { pages, hitCap, capReason } = await paginateSpApi(ctx, buildPath, (payload) => {
    ...
    // SP-API Finances: 0.5 rps sustained, burst 30. We want to stay
    // comfortably under — 2100ms between pages = ~0.47 rps. The 429
    // backoff in callSpApi handles bursts past the quota gracefully,
    // but on backfills we want to NOT blow the quota in the first
    // place.
    pageDelayMs: 2100,
  });
