...
function extractCatalogFields(res, marketplaceId) {
  ...
  /* images[0].images[].link → JSON array, MAIN first, canonical-only,
     up to 8 unique assets total.

     Catalog Items returns multiple URL variants per asset (high-res +
     pre-sized thumbnails like ._SL75_, ._AC_SX679_). uniqueAmazonAssets
     groups by asset ID and keeps the canonical (no size modifier) URL,
     so `41aK6YQOHeL.jpg` AND `41aK6YQOHeL._SL75_.jpg` collapse to one.
  ...
  */
  ...
  if (imgsMatch && Array.isArray(imgsMatch.images) && imgsMatch.images.length) {
    ...
    for (const img of imgsMatch.images) {
      if (img && img.variant === 'MAIN' && img.link) { ordered.push(img.link); break; }
    }
    // Then everything else, in API order
    for (const img of imgsMatch.images) {
      if (img && img.link) ordered.push(img.link);
    }
    const unique = uniqueAmazonAssets(ordered).slice(0, 8);
    if (unique.length) fields.ImagesJSON = JSON.stringify(unique);
  }
