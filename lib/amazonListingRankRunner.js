...
function extractCatalogFields(res, marketplaceId) {
  ...
  if (summary.brandName)        fields.Brand        = summary.brandName;
  ...
  // purchasable_offer — pick the retail node (no quantity_discount_type)
  const offers = Array.isArray(attrs.purchasable_offer) ? attrs.purchasable_offer : [];
  const retail = offers.find((o) => o && !o.quantity_discount_type) || offers[0];
  if (retail) {
    if (!fields.Currency && retail.currency) fields.Currency = retail.currency;
    // our_price[0].schedule[0].value_with_tax — the "what's on offer right now" price
    const ourPrice = Array.isArray(retail.our_price) ? retail.our_price[0] : null;
    const schedule = ourPrice && Array.isArray(ourPrice.schedule) ? ourPrice.schedule[0] : null;
    if (schedule) {
      const v = schedule.value_with_tax ?? schedule.value;
      if (Number.isFinite(Number(v))) {
        // If list_price wasn't set above, use this as Price
        if (fields.Price == null) fields.Price = Number(v);
        // If list_price was set AND it differs, treat this as SalePrice
        else if (Number(v) < fields.Price) fields.SalePrice = Number(v);
      }
      if (schedule.start_at) fields.SalePriceStart = schedule.start_at;
      if (schedule.end_at)   fields.SalePriceEnd   = schedule.end_at;
    }
    // discounted_price[0].schedule[0] — explicit promotional schedule
    const discounted = Array.isArray(retail.discounted_price) ? retail.discounted_price[0] : null;
    const dSchedule = discounted && Array.isArray(discounted.schedule) ? discounted.schedule[0] : null;
    if (dSchedule) {
      const v = dSchedule.value_with_tax ?? dSchedule.value;
      if (Number.isFinite(Number(v))) fields.SalePrice = Number(v);
      if (dSchedule.start_at) fields.SalePriceStart = dSchedule.start_at;
      if (dSchedule.end_at)   fields.SalePriceEnd   = dSchedule.end_at;
    }
  }
