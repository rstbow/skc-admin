 ...
 *
 * Rate limit aware: small delay between pages after the first to stay under
 * typical 0.5–2 rps caps. callSpApi handles 429s underneath.
 *
 * @param {object} ctx
 * @param {string} buildPath  Function: (nextToken | null) => '/path?query'
 * @param {function} onPage   Function: (payload) => void
 * @param {object} opts       { maxPages?, pageDelayMs? }
 */
async function paginateSpApi(ctx, buildPath, onPage, { maxPages = 200, pageDelayMs = 350 } = {}) {
  let nextToken = null;
  let pages = 0;
  let hitCap = false;
  do {
