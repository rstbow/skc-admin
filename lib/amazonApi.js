 ...
 */
async function callSpApi(ctx, path, { maxRetries = 10, method = 'GET', body = null } = {}) {
  ...
  while (true) {
    ...
    if (res.status === 429 && attempt < maxRetries) {
      // Respect Retry-After if present, else exponential backoff starting
      // at 2s. Cap at 60s per wait. For Finances (0.5 rps sustained) +
      // long backfills we'd rather wait patiently than blow up a run
      // that's already 80% done.
      const raHeader = parseFloat(res.headers.get('retry-after') || '');
      const backoff = Number.isFinite(raHeader) ? raHeader : Math.min(2 * Math.pow(2, attempt), 60);
      console.warn('[sp-api] 429 on attempt ' + (attempt + 1) + '/' + maxRetries +
        ', backing off ' + backoff.toFixed(1) + 's — path ' + path.slice(0, 80));
      await sleep(Math.min(60000, backoff * 1000));
      attempt++;
      continue;
    }
    if (!res.ok) {
      // Fold the body into the message so JobRuns.ErrorMessage is actually
      // diagnostic. Amazon's errors are shaped like:
      //   {"errors":[{"code":"InvalidInput","message":"...","details":"..."}]}
      // Pull the first error for the summary; keep full body on err.response.
      let summary = '';
      if (json && Array.isArray(json.errors) && json.errors.length) {
        ...
        summary = ' — ' + (firstError.code || '')
                ...
                + (firstError.message || '');
        if (firstError.details) summary += ' (' + String(firstError.details).slice(0, 200) + ')';
      } else if (text) {
        summary = ' — ' + String(text).slice(0, 300);
      }
      const err = new Error('SP-API call failed: ' + res.status
