...
const { sql, getProjectPool } = require('../../config/db');
...
async function handler(ctx) {
  ...
  //    table in vs-ims-project. Azure SQL Database doesn't support
  //    cross-DB 4-part references, so we use a dedicated project pool
  //    (config/db.js#getProjectPool — defaults its credentials to the
  //    STAGING_DB_USER/PASSWORD values since skc_app_user typically
  //    has SELECT on both vs-ims-staging and vs-ims-project).
  let projectPool;
  try {
    projectPool = await getProjectPool();
  } catch (e) {
    const isUnconfigured = /not configured/i.test(e.message);
    log('ERROR', isUnconfigured ? 'project pool not configured' : 'project pool connect failed', { error: e.message });
    return {
      status:           'ERROR',
      summary:          isUnconfigured
                          ? 'Project DB pool not configured (set PROJECT_DB_USER/PROJECT_DB_PASSWORD or fall back via STAGING_DB_USER/PASSWORD)'
                          : 'Project DB pool connect failed: ' + e.message,
      metadata:         { defaultDaysBack, brandsObserved: 0 },
      errorMessage:     e.message,
      errorFingerprint: isUnconfigured ? 'project-pool-unconfigured' : 'project-pool-connect-failed',
    };
  }

  let stateRows;
  try {
    const r = await projectPool.request().query(`
      SELECT TOP 200
             brand_id,
             Report1Active,
             Report1DataStart
        FROM dbo.tbl_PPA_SP_API_Report_Runs
