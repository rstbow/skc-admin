...
async function handler(ctx) {
  ...
  // 1. Pull current per-brand source-window state from the staging-side
  //    source-runs table. This is the table the SP-API SSIS package
  //    reads to determine its window when pulling reports.
  let stagingPool;
  try {
    stagingPool = await getStagingPool();
  } catch (e) {
    log('ERROR', 'staging pool not configured', { error: e.message });
    return {
      status:           'ERROR',
      summary:          'Staging DB pool not configured (set STAGING_DB_USER/STAGING_DB_PASSWORD env vars)',
      metadata:         { defaultDaysBack, brandsObserved: 0 },
      errorMessage:     e.message,
      errorFingerprint: 'staging-pool-unconfigured',
    };
  }
  ...
  try {
    ...
    return {
      status:           'ERROR',
      summary:          'Failed to read source-runs state from staging: ' + e.message,
      metadata:         { defaultDaysBack },
      errorMessage:     e.message,
      errorFingerprint: 'staging-query-failed',
    };
  }
