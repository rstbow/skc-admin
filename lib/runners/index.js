...
const { runAmazonListingRank }     = require('../amazonListingRankRunner');
const { runAmazonOrders }          = require('../amazonOrdersRunner');
const { runAmazonReturns }         = require('../amazonReturnsRunner');
...
async function amzListingRankAdapter(ctx) {
  const result = await runAmazonListingRank({
    credentialID: ctx.credentialID,
    triggeredBy:  ctx.triggeredBy || 'SCHEDULE',
    userID:       ctx.userID || null,
    params:       ctx.params || {},
  ...
  return {
    ...
    status:         result.ok === false ? 'FAILED' : 'SUCCESS',
    rowsIngested:   result.eventsProcessed ?? null,
    message:        result.asinsQueried != null
      ? (result.asinsQueried + ' ASINs queried, ' +
         (result.rowsInserted + result.rowsUpdated) + ' rank rows written')
      : null,
    wroteOwnJobRun: true,
  };
}

/* Adapter for the Orders runner — passes Params through.
   Two pull modes selectable via params.mode = 'CREATED' (recent sales) or
   'UPDATED' (catch status flips on existing orders). Default CREATED.
*/
async function amzOrdersAdapter(ctx) {
  const result = await runAmazonOrders({
    credentialID: ctx.credentialID,
    triggeredBy:  ctx.triggeredBy || 'SCHEDULE',
    userID:       ctx.userID || null,
    params:       ctx.params || {},
  ...
  return {
    runID:          result.runID || ctx.runID,
    status:         result.ok === false ? 'FAILED' : 'SUCCESS',
    rowsIngested:   (result.ordersProcessed ?? 0) + (result.itemsProcessed ?? 0),
    message:        result.ordersProcessed != null
      ? (result.ordersProcessed + ' orders, ' + result.itemsProcessed + ' items (' + result.mode + ' mode)')
      : null,
    wroteOwnJobRun: true,
  };
}

/* Adapter for the FBA Returns runner. */
async function amzReturnsAdapter(ctx) {
  const result = await runAmazonReturns({
    credentialID: ctx.credentialID,
    triggeredBy:  ctx.triggeredBy || 'SCHEDULE',
    userID:       ctx.userID || null,
    params:       ctx.params || {},
  ...
  return {
    runID:          result.runID || ctx.runID,
    status:         result.ok === false ? 'FAILED' : 'SUCCESS',
    rowsIngested:   result.returnsProcessed ?? null,
    message:        result.returnsProcessed != null
      ? (result.returnsProcessed + ' return rows (' + result.windowDays + 'd window)')
      : null,
    wroteOwnJobRun: true,
  };
}
...
const registry = {
  ...
  AMZ_LISTING_RANK_SNAPSHOT:  amzListingRankAdapter,
  AMZ_ORDERS:                 amzOrdersAdapter,
  AMZ_RETURNS:                amzReturnsAdapter,
