...
async function getBrandPool(brandUID) {
  ...
  // requestTimeout history:
  //   30s   → too tight, ZenToes hit it after 36 min of work
  //   120s  → still too tight, ZenToes 180d backfill MERGE batches in
  //           hot-lock chunks blew past it. The cancel itself then
  //           timed out at the default 5s cancelTimeout, surfacing as
  //           "Failed to cancel request in 5000ms" — meaning SQL was so
  //           busy it couldn't even ack the abort.
  //   240s  → current. Stays under the scheduler's 1h concurrency lock
  //           with margin, but gives a hot-lock MERGE batch a real
  //           chance to land.
  // Combined with BATCH_SIZE drop (500→250 in financialEventsRunner)
  // and chunkDays=2 on backfills, big-brand backfills should land
  // cleanly. If a brand still flakes, pass requestTimeout in its
  // DataDbConnString JSON to override.
  cfg.requestTimeout = cfg.requestTimeout || 240000;
  cfg.connectionTimeout = cfg.connectionTimeout || 30000;
  // Bump cancelTimeout from default 5s → 30s. When requestTimeout fires,
  // mssql sends an attention packet and waits cancelTimeout for an ack.
  // 5s wasn't enough for genuinely lock-contended calls, surfacing as
  // a confusing secondary error. 30s is patient enough that the real
  // primary timeout error wins.
  cfg.options = cfg.options || {};
  if (cfg.options.cancelTimeout == null) cfg.options.cancelTimeout = 30000;
