...
async function start() {
  ...
  // Recover orphaned runs first — App Service deploys / crashes leave
  // JobRuns rows stuck in RUNNING with no live process. Marking them
  // FAILED releases the concurrency gate so cron can fire again.
  await sweepStaleRuns().catch((e) =>
    console.warn('[scheduler] stale-run sweep failed (non-fatal):', e.message));
  await reload();
  console.log('[scheduler] ready, ' + scheduled.size + ' NODE_NATIVE jobs scheduled');
}

/**
 * Mark any RUNNING JobRuns row older than the threshold as FAILED.
 * Called on scheduler.start() so worker recycles auto-recover. Conservative
 * threshold (90 min) — longer than any healthy run we have today.
 */
  ...
  const r = await pool.request().query(`
    UPDATE admin.JobRuns
       SET EndedAt      = SYSUTCDATETIME(),
           Status       = 'FAILED',
           ErrorMessage = CONCAT('Auto-marked FAILED on scheduler boot — ',
                                 'JobRuns row was orphaned by a worker recycle. ',
                                 'StartedAt was ',
                                 CAST(StartedAt AS NVARCHAR(30)),
                                 ', detected ',
                                 CAST(SYSUTCDATETIME() AS NVARCHAR(30)),
                                 '.')
     OUTPUT INSERTED.RunID
     WHERE Status = 'RUNNING'
       AND StartedAt < DATEADD(MINUTE, -90, SYSUTCDATETIME());
  `);
  if (r.recordset && r.recordset.length) {
    console.log('[scheduler] swept ' + r.recordset.length + ' stale RUNNING JobRuns row(s) — RunIDs ' +
      r.recordset.map((x) => x.RunID).join(','));
  }
}
...
async function fire(jobID, { triggeredBy, userID } = {}) {
  ...
  // Also pulls endpoint defaults so we can merge: brand-level Params override
  // endpoint defaults per key. A job with Params=null inherits the endpoint
  // defaults entirely.
  const j = await pool.request()
    ...
    .query(`
      SELECT j.JobID, j.Name, j.BrandUID, j.EndpointID, j.ConcurrencyKey,
             j.IsActive, j.Params,
             e.Name AS EndpointName, e.DefaultParams AS EndpointDefaultParams,
             c.Name AS ConnectorName,
  ...
  // Rapid-fire guard: reject if the same job had a run start within the
  // last 5 seconds (any status). Catches double-clicks from the UI where
  // the first run already completed/failed by the time the second click
  // hits the server, so the RUNNING-row gate above wouldn't trigger.
  const recent = await pool.request()
    .input('jid', sql.Int, jobID)
    .query(`
      SELECT TOP 1 RunID, StartedAt, Status FROM admin.JobRuns
      WHERE JobID = @jid AND StartedAt > DATEADD(SECOND, -5, SYSUTCDATETIME())
      ORDER BY StartedAt DESC
    `);
  if (recent.recordset.length) {
    const r = recent.recordset[0];
    throw new Error('Job ' + jobID + ' was just fired (RunID ' + r.RunID +
      ', status=' + r.Status + ') — wait a few seconds between retries');
  }
  ...
  // Merge endpoint defaults + job-level overrides per key. Brand-level
  // Params win when both are set. Malformed JSON on either side is
  // logged but doesn't fail the run — the runner falls back to its
  // own hard-coded defaults.
  let endpointDefaults = {};
  if (job.EndpointDefaultParams) {
    try { endpointDefaults = JSON.parse(job.EndpointDefaultParams); }
    catch (e) {
      console.warn('[scheduler] endpoint ' + job.EndpointName +
        ' has malformed DefaultParams JSON, ignoring: ' + e.message);
    }
  }
  let jobOverrides = {};
  if (job.Params) {
    try { jobOverrides = JSON.parse(job.Params); }
    catch (e) {
      console.warn('[scheduler] job ' + jobID +
        ' has malformed Params JSON, ignoring: ' + e.message);
    }
  }
  const params = { ...endpointDefaults, ...jobOverrides };
