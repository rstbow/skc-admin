...
function scheduleOne(job) {
  ...
  }, { timezone: job.TimezoneIANA || 'America/Chicago' });

  scheduled.set(job.JobID, { task, cronExpr: expr, timezone: job.TimezoneIANA, job });
  console.log('[scheduler] scheduled job ' + job.JobID + ' "' + job.Name + '" @ ' + expr +
    ' (' + (job.TimezoneIANA || 'America/Chicago') + ')');
}
...
async function fire(jobID, { triggeredBy, userID } = {}) {
  ...
  if (!job.IsActive) {
    throw new Error('Job ' + jobID + ' is paused (IsActive=0) — re-enable before running');
  }
