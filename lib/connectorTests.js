...
/* ========================================================================== */
/*  SKU Compass — internal connector; connectivity test against brand DB       */
/* ========================================================================== */
async function testSkuCompass({ cred }) {
  // Expects cred.BrandUID to be set (loaded by the caller)
  const brandUID = cred.BrandUID;
  if (!brandUID) {
    return { ok: false, message: 'BrandUID missing on credential context.' };
  }
  try {
    const pool = await getBrandPool(brandUID);
    const r = await pool.request().query(`
      SELECT
        (SELECT COUNT(*) FROM sys.tables WHERE name = 'tbl_PPA_IMS_SKU') AS HasSkuTable,
        (SELECT TOP 1 name FROM sys.columns
          WHERE object_id = OBJECT_ID('tbl_PPA_IMS_SKU')
            AND name IN ('COG','Cost','UnitCost','COGS','Item_COG','Unit_Cost')) AS CogColumn;
    `);
    const row = r.recordset[0] || {};
    if (!row.HasSkuTable) {
      return {
        ok: false,
        message: 'Connected to brand DB, but tbl_PPA_IMS_SKU does not exist. COG pulls will not work until it is created.',
      };
    }
    if (!row.CogColumn) {
      return {
        ok: true,
        partial: true,
        message: 'Connected to brand DB. tbl_PPA_IMS_SKU exists but no recognizable COG column was found.',
        details: { hasSkuTable: true, cogColumn: null },
      };
    }
    return {
      ok: true,
      message: 'Connected. tbl_PPA_IMS_SKU found with COG column "' + row.CogColumn + '".',
      details: { hasSkuTable: true, cogColumn: row.CogColumn },
    };
  } catch (e) {
    return { ok: false, message: 'Brand DB connection failed: ' + e.message, details: { code: e.code } };
  }
}
