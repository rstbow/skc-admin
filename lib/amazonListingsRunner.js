...
async function loadCredentialContext(credentialID) {
  ...
  const r = await pool.request()
    ...
    .query(`
      SELECT bc.CredentialID, bc.BrandUID, bc.ConnectorID,
             ...
             bc.RefreshToken, bc.AccessToken, bc.TokenExpiresAt,
             c.Name AS ConnectorName, c.DisplayName AS ConnectorDisplay,
             c.AuthType, c.BaseURL, c.AppClientID, c.AppClientSecret,
  ...
  return {
    ...
    connector: { ConnectorID: row.ConnectorID, Name: row.ConnectorName, DisplayName: row.ConnectorDisplay,
                 ...
                 AppClientID: row.AppClientID, AppClientSecret: row.AppClientSecret },
    cred: { Region: row.Region, MarketplaceIDs: row.MarketplaceIDs, SellerID: row.SellerID,
            RefreshToken: row.RefreshToken, AccessToken: row.AccessToken,
