...
const { uniqueAmazonAssets } = require('./amazonImages');
...
function normalizeReportRow(r, marketplaceId) {
  ...
  return {
    ...
    Category:      null,
    BrowseNodeID:  null,
    // CRITICAL: when the flat-file row has no image-url, leave ImagesJSON
    // null so the merge proc's `ISNULL(src.ImagesJSON, tgt.ImagesJSON)`
    // preserves whatever the rank runner's catalog enrichment populated.
    // Writing '[]' (a JSON-serialized empty array) is non-null and would
    // overwrite the rich images. This was the regression app2 caught
    // 2026-04-25.
...
async function enrichOne(ctx, row, marketplaceId) {
  // includedData=offers gets us purchasable_offer (sale price + schedule).
  // The flat-file report only carries the seller's regular price; the
  // buyer-facing sale price lives in offers.purchasable_offer.
  ...
  applyListingsItemsToRow(row, res, marketplaceId);
}

function applyListingsItemsToRow(row, apiRes, marketplaceId) {
  const summaries = Array.isArray(apiRes && apiRes.summaries) ? apiRes.summaries : [];
  // Listings Items returns one summary per marketplace; pick the matching
  ...
  const offerNodes = Array.isArray(apiRes && apiRes.offers) ? apiRes.offers : [];

  if (summary.asin) row.ASIN = summary.asin;
  if (summary.itemName)  row.Title = summary.itemName;
  // Brand precedence: summary.brandName → attributes.brand[0].value.
  // Tessa's + ZenToes populate summary.brandName; Earth's Daughter +
  // Rengora's ASINs only have brand under attributes.brand even though
  // their titles clearly show the brand name. Fall back to the
  // attribute path so all brands populate consistently.
  if (summary.brandName) row.Brand = summary.brandName;
  else {
    const attrBrand = firstAttrValue(attributes, 'brand');
    if (attrBrand) row.Brand = String(attrBrand).slice(0, 200);
  }
  ...
  // Listings Items API generally returns canonical URLs, but Amazon
  // occasionally slips in a sized variant (._SL75_, ._AC_SX679_, etc).
  // uniqueAmazonAssets collapses any duplicates by asset ID and prefers
  // the canonical (no size modifier) variant — same filter applied in
  // the rank runner. Filed by app2 in handoff
  // 2026-04-25-listings-images-thumbnail-variants.md.
  const imgs = [];
  ...
  const uniqueImgs = uniqueAmazonAssets(imgs);
  if (uniqueImgs.length) row.ImagesJSON = JSON.stringify(uniqueImgs);
  ...
  // Sale price + schedule — Listings Items API offers[].purchasable_offer
  // is the authoritative source for what the buyer sees. The flat-file
  // report's `price` column is the seller's regular price (List/MSRP);
  // when there's an active sale, that diverges from what's displayed
  // on Amazon.com. We populate Price (regular) AND SalePrice (active
  // schedule) so app2 can show "$22.97 ~~$19.97~~ (sale ends Apr 30)".
  applySalePriceFromOffers(row, attributes, offerNodes, marketplaceId);
...
/**
 * Pull regular + sale price + schedule from a Listings Items API response.
 * Two possible locations:
 *   - apiRes.offers[].marketplaceId == X.purchasable_offer  (preferred — direct from /offers includedData)
 *   - apiRes.attributes.purchasable_offer[]                (fallback if offers includedData wasn't honored)
 *
 * Sale-price precedence (per skc-api 2026-04-24 handoff):
 *   1. Retail tier only — skip nodes with quantity_discount_type set
 *   2. discounted_price[0].schedule[0] is the explicit promotional price.
 *      If absent, our_price[0].schedule[0] below list_price is treated
 *      as a sale.
 *
 * Sets row.Price / row.SalePrice / row.SalePriceStart / row.SalePriceEnd
 * / row.Currency. Doesn't clobber if the value's already populated and
 * the offer doesn't have a richer one.
 */
function applySalePriceFromOffers(row, attributes, offerNodes, marketplaceId) {
  // Find the right purchasable_offer block. Prefer offers includedData;
  // fall back to attributes.purchasable_offer.
  let pos = null;
  if (offerNodes.length) {
    const match = offerNodes.find((o) => o && o.marketplaceId === marketplaceId) || offerNodes[0];
    if (match && Array.isArray(match.purchasable_offer)) pos = match.purchasable_offer;
  }
  if (!pos) {
    const fromAttr = pickAttr(attributes, 'purchasable_offer');
    if (fromAttr) pos = fromAttr;
  }
  if (!pos || !pos.length) return;

  const retail = pos.find((o) => o && !o.quantity_discount_type) || pos[0];
  if (!retail) return;
  if (!row.Currency && retail.currency) row.Currency = retail.currency;

  // our_price = the seller's regular offer price
  const ourPrice = Array.isArray(retail.our_price) ? retail.our_price[0] : null;
  const ourSched = ourPrice && Array.isArray(ourPrice.schedule) ? ourPrice.schedule[0] : null;
  const ourValue = ourSched ? (ourSched.value_with_tax ?? ourSched.value) : null;

  // discounted_price = explicit promotional schedule
  const discounted = Array.isArray(retail.discounted_price) ? retail.discounted_price[0] : null;
  const discSched  = discounted && Array.isArray(discounted.schedule) ? discounted.schedule[0] : null;
  const discValue  = discSched ? (discSched.value_with_tax ?? discSched.value) : null;

  // Set the buyer-facing live price as Price. If a discounted schedule
  // is active, that's SalePrice + start/end. Otherwise our_price IS
  // the live price (no sale).
  if (Number.isFinite(Number(discValue))) {
    row.SalePrice      = Number(discValue);
    row.SalePriceStart = discSched.start_at || null;
    row.SalePriceEnd   = discSched.end_at   || null;
    if (Number.isFinite(Number(ourValue))) row.Price = Number(ourValue);
  } else if (Number.isFinite(Number(ourValue))) {
    // No promo schedule — our_price is what buyers see. Mirror to Price.
    row.Price = Number(ourValue);
  }
}
...
// Which fields carry a delta worth tracking from THIS runner's source
// (the GET_MERCHANT_LISTINGS_ALL_DATA flat-file report).
//
// CRITICAL — keep this list aligned to what the flat-file report
// actually returns. Bullets / Brand / SearchTerms / Category / Images /
// BrowseNodeID / ProductType are NOT in the report; they're only
// populated by the rank runner's Catalog Items enrichment. If we
// included them here, every daily listings run would see
// cur=populated (from yesterday's rank run) vs fresh=null (this
// runner's source doesn't have them) and emit false-positive "value
// cleared" change rows — even though the merge proc preserves the
// existing values via ISNULL(src.X, tgt.X). That's the bug app2
// reported on 2026-04-25; this list is the fix.
//
// Catalog-only fields (bullets, brand, etc.) belong in the rank
// runner's own diff step (TODO).
